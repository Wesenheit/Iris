...
from .bijectors import transform, untransform, biject, Identity, Exp
import h5py
...
def get_Vista(name):
    wave = (
        np.loadtxt(
            directory.joinpath("VISTA_Filters_at80K_forETC_" + name + ".dat"),
            unpack=True,
            usecols=[0],
            dtype=float,
        )
...
def get_xmm(name):
    wave = (
        np.loadtxt(
            directory.joinpath("XMM_OM." + name + ".dat"),
            unpack=True,
            usecols=[0],
            dtype=float,
        )
...
def get_Denis(name):
    wave = (
        np.loadtxt(
            directory.joinpath("DENIS_DENIS." + name + ".dat"),
            unpack=True,
            usecols=[0],
            dtype=float,
        )
...
def get_IRSF(name):
    wave = (
        np.loadtxt(
            directory.joinpath("IRSF_Sirius.{}.dat".format(name)),
            unpack=True,
            usecols=[0],
            dtype=float,
        )
...
def get_SWIFT(name):
    wave = (
        np.loadtxt(
            directory.joinpath("Swift_UVOT." + name + ".dat"),
            unpack=True,
            usecols=[0],
            dtype=float,
        )
...
def get_UKIDSS(name):
    wave = (
        np.loadtxt(
            directory.joinpath("UKIRT_UKIDSS." + name + ".dat"),
            unpack=True,
            usecols=[0],
            dtype=float,
        )
...
def get_H_Alpha():
    wave = (
        np.loadtxt(
            directory.joinpath("INT_IPHAS.Ha.dat"),
            unpack=True,
            usecols=[0],
            dtype=float,
        )
...
def get_GAIA_DR3(id):
    wave = (
        np.loadtxt(
            directory.joinpath("passband.dat"), unpack=True, usecols=[0], dtype=float
        )
        * 10
        * unit["AA"]
    )
    trans = np.loadtxt(
        directory.joinpath("passband.dat"),
        unpack=True,
        usecols=[1 + 2 * id],
        dtype=float,
    )
...
class Star:
    ...
    def __init__(
        ...
        E_B_V: Optional[float] = None,
        Z: Optional[float] = 0.013,
        RV: Optional[float] = 3.1,
    ):
        self.name = name  # name
    ...
    def get_Halpha_value(self, single=True, ax=None, use_AB=False, **kwargs):
        ...
        if len(result) == 0:
            ...
            if ax is not None:
                _lam, _F_nu, _F_lam, _lamF_lam, _frac_error = mag_to_flux_pyphot(
                    self.OTHER["Ha"], Ha, Ha_err, False
                )
                if use_AB:
                    ax.errorbar(
                        [_lam],
                        [-2.5 * np.log10(_F_nu / 3630.8)],
                        [Ha_err],
                        fmt=".",
                        color="red",
                        label=r"$H_{\alpha}$",
    ...
    def get_BailerJohns2020_prior(self):
        helper = np.loadtxt(
            directory.joinpath("HEALpixel_level5_radec_longlat_coordinates.csv"),
            skiprows=1,
            delimiter=",",
        )
        target_data = np.loadtxt(
            directory.joinpath("prior_summary.csv"), skiprows=1, delimiter=","
        )
        sky_distances = (
            angular_separation(
                self.ra * u.deg,
        ...
        self.d_prior = (
            lambda x: -scipy.special.loggamma((self.beta + 1) / self.alpha)
            + np.log(
                self.alpha * np.power(self.L, -self.beta - 1) * np.power(x, self.beta)
            )
            - np.power(x / self.L, self.alpha)
        )
        self.parallax_prior = lambda x: -((self.plx - 1 / x) ** 2) / (
            2 * self.e_plx**2
        ) + self.d_prior(x)

    def set_Bayestar_EBV_est(
        self, downloaded=False, version="bayestar2019", dmax=20, N=50, mode="median"
    ):
        if downloaded:
        ...
        def estimate(d, dmax):
            if len(self.ebv_est.shape) > 1:
                id_to_choose = np.random.randint(
                    0, self.ebv_est.shape[1], size=self.d_arr_ebv.shape[0]
                )
    ...
    def get_GALEX(self, num=0):
        results = Catalogs.query_object(
            ...
            catalog="Galex",
            radius=u.arcsec * 2 * self.dis_norm,
        )
        if len(results) > 0:
        ...
        if len(file) > 0:
            ...
            for i in range(len(self.catalogs[name][0]) // 2):
                if str(file[num][self.catalogs[name][0][2 * i]]) == "nan":
                    ...
                    err = (
                        0
                        if str(file[num][self.catalogs[name][0][2 * i + 1]]) == "nan"
                        else float(file[num][self.catalogs[name][0][2 * i + 1]])
                    )
                    name_new = (
                        self.con[(self.catalogs[name][1])[i]]
                        if (self.catalogs[name][1])[i] in self.con
                        else (self.catalogs[name][1])[i]
                    )
                    if name_new not in self.filters:
                        ...
                        self.ampl = np.append(
                            self.ampl, float(file[num][self.catalogs[name][0][2 * i]])
                        )
                        self.err = np.append(self.err, err)
                        self.err_estimate_flag = np.append(
                            self.err_estimate_flag, False
                        )
        else:
        ...
        try:
            ...
            for i in range(len(self.catalogs[name][0]) // 2):
                if str(file[num][2 * i]) == "--":
                    ...
                    err = (
                        0
                        if str(file[num][2 * i + 1]) == "--"
                        else float(file[num][2 * i + 1])
                    )
                    name_new = (
                        self.con[(self.catalogs[name][1])[i]]
                        if (self.catalogs[name][1])[i] in self.con
                    ...
                    if name_new not in self.filters:
                        ...
                        self.err = np.append(self.err, err)
                        self.err_estimate_flag = np.append(
                            self.err_estimate_flag, self.catalogs[name][3][i]
                        )
        except IndexError:
            pass
        ...
        try:
            ...
            for i in range(5):
                if str(file[num][2 * i]) == "--":
                    ...
                    check_bit(file[num][10 + i], [5, 9, 46, 47, 40])
                    and is_set(file[num][10 + i], 28)
                    and not (
                        is_set(file[num][10 + i], 28) or is_set(file[num][10 + i], 34)
                    )
                ):
                    print("Wrong bits!")
                else:
                    ...
                    name_new = (
                        self.con[names2[i]] if names2[i] in self.con else names2[i]
                    )
                    if name_new not in self.filters:
                        if i == 0:
                            corr = -0.04
        ...
        for i, (err, name) in enumerate(zip(self.err, self.filters)):
            if err <= 0:
                if replace == "max":
                    ...
                    print(
                        "filter {} with no magnitude error! Setting to max".format(name)
                    )
                elif type(replace) is float:
                    self.err[i] = replace
                    print(
                        "filter {} with no magnitude error! Setting to {}".format(
                            name, replace
                        )
                    )
                else:
    ...
    def set_EBV(self, ebv, verbose=True):
        ...
        self.ext = extinction.fitzpatrick99(
            np.array(self.lib_stell.wavelength).astype(np.double),
            self.RV * self.EBV,
            self.RV,
        )
        if verbose:
    ...
    def add_obs(self, name, ampl, err, flag=True):
        ...
        """
        self.filters = np.append(
            self.filters, self.con[name] if name in self.con else name
        )
    ...
    def get_ebv_green19(self, downloaded=False, version="bayestar2019", mode="median"):
        coords = SkyCoord(
            ra=self.ra * u.deg,
            dec=self.dec * u.deg,
            distance=self.d * u.kpc if not self.use_parallax else 1 / self.plx * u.kpc,
            frame="icrs",
        )
        if downloaded:
            ...
            if show_flag:
                print(
                    "{} {:.5g} {:.2g} {}".format(
                        ...
                        self.ampl[i],
                        self.err[i],
                        self.err_estimate_flag[i],
                    )
                )
            else:
                print(
                    "{} {:.5g} {:.2g}".format(
                        self.filters[i], self.ampl[i], self.err[i]
                    )
                )
    ...
    def predict_single(self, list_filters):
        ...
        if self.Z is None:
            ...
            Z = self.Z
        stell = self.lib_stell.generate_stellar_spectrum(
            self.par_single[0], g, self.par_single[1], Z
        ) / (4 * math.pi * d**2 * kpc**2)
        if self.EBV is not None:
            stell = np.power(10, -0.4 * self.ext) * stell
        for i in range(len(list_filters)):
            try:
                val = self.lib_phot[list_filters[i]].get_flux(
                    np.array(self.lib_stell.wavelength) * unit["AA"],
                    np.array(stell) * unit["flam"],
                )
                if list_filters[i] in self.AB:
                    pred[i] = (
                        -2.5 * np.log10(val.value)
                        - self.lib_phot[list_filters[i]].AB_zero_mag
                    )
                else:
                    pred[i] = (
                        -2.5 * np.log10(val.value)
                        - self.lib_phot[list_filters[i]].Vega_zero_mag
                    )
            except AttributeError:
                val = self.OTHER[list_filters[i]].get_flux(
                    np.array(self.lib_stell.wavelength) * unit["AA"],
                    np.array(stell) * unit["flam"],
                )
                if list_filters[i] in self.AB:
                    pred[i] = (
                        -2.5 * np.log10(val) - self.OTHER[list_filters[i]].AB_zero_mag
                    )
                else:
                    pred[i] = (
                        -2.5 * np.log10(val) - self.OTHER[list_filters[i]].Vega_zero_mag
                    )
        return pred
    ...
    def predict_double(self, list_filters):
        ...
        d = self.par_double[4]
        stell = (
            self.lib_stell.generate_stellar_spectrum(
                self.par_double[0], self.g1, self.par_double[1], self.Z1
            )
            + self.lib_stell.generate_stellar_spectrum(
                self.par_double[2], self.g2, self.par_double[3], self.Z2
            )
        ) / (4 * math.pi * d**2 * kpc**2)
        if self.EBV is not None:
            stell = np.power(10, -0.4 * self.ext) * stell
    ...
    def get_log_prob_full(self, in_data, pred_ret=False):
        ...
        for i in range(len(self.ampl)):
            val = self.fil_obj[i].get_flux(
                self.lib_stell.wavelength.magnitude * unit["AA"],
                stell * unit["flam"],
                axis=1,
            )
            try:
        ...
        return_val = (
            -np.sum(
                (pred * factor - self.fluxes_) ** 2 / (2 * self.err_flux**2), axis=1
            )
            - (RV - self.RV_mean) ** 2 / (2 * self.RV_std**2)
            + np.log(self.AV_exp)
            - self.AV_exp * AV
        )
    ...
    def get_log_prob_simple(
        self, in_data, use_Z=False, estimate_error=None, no_D=False, estimate_EBV=False
    ):
        """
        log prob of observations giving single star model
        """
        ...
        if in_data.shape[1] > 3:
            if use_Z:
                ...
                logT_low, logT_high = np.array(
                    list(map(lambda x: self.get_boundaries(x), g))
                ).T
                # if (logT < 1).any():
        ...
        dictionary = {}
        dictionary["logT"] = logT
        dictionary["logg"] = g * np.ones_like(logT) if type(g) is float else g
        dictionary["logL"] = logL
        dictionary["Z"] = Z * np.ones_like(logT) if not use_Z else Z
        tab = QTable(dictionary)
        stell = (
            self.lib_stell.generate_individual_spectra(tab)[1].magnitude.T
            / (4 * math.pi * d**2 * kpc**2)
        ).T
        if estimate_EBV:
            ...
            for i in range(ext.shape[0]):
                ext[i, :] = extinction.ccm89(
                    self.lib_stell.wavelength.magnitude, av=self.RV * ebv[i], rv=self.RV
                )
            stell = np.power(10, -0.4 * ext) * stell
        else:
            if self.EBV is not None:
        ...
        if estimate_error is None:
            ...
            for i in range(len(self.err)):
                if self.err_estimate_flag[i]:
                    errors[:, i] = np.sqrt(
                        (pred[:, i] * self.error_estimate[1] * np.log(10) * 0.4) ** 2
                        + self.err_flux[i] ** 2
                    )
                else:
    ...
    def get_log_prob_double(
        self, in_data, limit=False, estimate_error=None, no_D=False
    ):
        """
        ...
        stell = (
            self.lib_stell.generate_individual_spectra(tab)[1].magnitude.T
            / (4 * np.pi * np.concatenate((d, d)) ** 2 * kpc**2)
        ).T
        if self.EBV is not None:
        ...
        if estimate_error is None:
            ...
            flux = (
                self.error_estimate[0].get_flux(
                    self.lib_stell.wavelength.magnitude * unit["AA"],
                    stell * unit["flam"],
                )
            ) / self.zerop[i]
    ...
    def get_bic_double(self, **kwargs):
        chi2 = (
            -2
            * self.get_log_prob_double(
                np.array(self.par_double).reshape(1, -1), no_D=True, **kwargs
            )[0]
        )
    ...
    def get_bic_simple(self, **kwargs):
        chi2 = (
            -self.get_log_prob_simple(
                self.par_single.reshape(1, -1), no_D=True, **kwargs
            )[0]
            * 2
        )
    ...
    def run_chain_double(
        ...
        **kwargs,
        ...
        if start is None:
            ...
            if kwargs["limit"]:
                if logT1_high < logT2_low:
                    ...
                    high = min(logT1_high, logT2_high)
                    low = max(logT2_low, logT1_low)
                    start[:, 2] = np.random.rand(n) * (high - low) + low
                    start[:, 0] = (
                        np.random.rand(n) * (logT1_high - start[:, 2]) + start[:, 2]
                    )
        else:
            if len(start.shape) == 1:
                start = (
                    np.repeat(np.array([start]), n, axis=0)
                    + np.random.randn(n, 5) * 0.01
                )

        print("starting conditions: ", start)

        bijector_list_double = [
            Identity(logT1_low, logT1_high),
            Identity(logl_low, logl_high),
            Identity(logT2_low, logT2_high),
            Identity(logl_low, logl_high),
            Exp(),
        ]
        ...
        sampler = emcee.EnsembleSampler(
            n,
            5,
            (biject(bijector_list_double))(self.get_log_prob_double),
            kwargs=kwargs,
            vectorize=True,
        )
        ...
        if rerun:
            temp = untransform(
                sampler.get_chain(flat=True, discard=num_burn), bijector_list_double
            )
            ...
            sampler.reset()
            sampler.run_mcmc(
                transform(new_start, bijector_list_double), num_step, progress=progress
            )
        temp = untransform(
            sampler.get_chain(flat=True, discard=num_burn), bijector_list_double
        )
        ...
        self.par_double = [
            np.median(logT1_samples),
            np.median(logL1_samples),
            np.median(logT2_samples),
            np.median(logL2_samples),
            np.median(d_samples),
        ]
    ...
    def run_chain_full(
        ...
        **kwargs,
        ...
        bijector_list_sig = [
            Identity(np.min(self.lib_stell.logT), np.max(self.lib_stell.logT)),
            Identity(logg_low, logg_high),
            Identity(Z_low, Z_high),
            Identity(av_low, av_high),
            Identity(RV_low, RV_high),
        ]
        sampler = emcee.EnsembleSampler(
            n,
            5,
            (biject(bijector_list_sig))(self.get_log_prob_full),
            kwargs=kwargs,
            vectorize=True,
        )
        ...
        if start is None:
            ...
            start[:, 2] = 10 ** (
                np.random.rand(n) * (np.log10(Z_high) - np.log10(Z_low))
                + np.log10(Z_low)
            )
        ...
        if rerun:
            temp = untransform(
                sampler.get_chain(flat=True, discard=num_burn), bijector_list_sig
            )
            temp_tran = np.unique(temp, axis=0)
            print("sampling new starting conditions")
            id = np.random.permutation(len(temp_tran))[:n]
            ...
            sampler.reset()
            sampler.run_mcmc(
                transform(new_start, bijector_list_sig), num_step, progress=progress
            )
        ...
        print(points)
        chi2, pred = self.get_log_prob_full(
            transform(points.reshape(1, -1), bijector_list_sig), pred_ret=True
        )
    ...
    def run_chain_simple(
        ...
        **kwargs,
        ...
        sampler = emcee.EnsembleSampler(
            n,
            3,
            (biject(bijector_list_sig))(self.get_log_prob_simple),
            kwargs=kwargs,
            vectorize=True,
        )
        ...
        if start is None:
            ...
            if self.use_parallax:
                start[:, 2] = np.clip(
                    1 / (np.random.randn(n) * self.e_plx + self.plx), 0.001, 8
                )
            else:
        ...
        self.par_single = np.array(
            [np.median(logT_samples), np.median(logL_samples), np.median(val_samples)]
        )
        ...
        **kwargs,
        ...
        if start is None:
            ...
            if self.use_parallax:
                ...
                start[:, 2] = np.random.randn(n) * self.d_err + self.d
            start[:, 3] = Z_start * (
                1 + np.random.randn(n) / 20
            )  # np.random.rand(n)*(z_high-z_low)+z_low
        else:
            if len(start.shape) == 1:
                logT_low, logT_high = self.get_boundaries(start[3])
                start = (
    ...
    def plot_measurments(
        self, ax, plot_fwhm=False, plot_ebv=True, limit=2, errors=None, use_AB=False
    ):
        N = len(self.ampl)
        ...
        for i in range(N):
            ...
            if errors is None:
                _lam, _F_nu, _F_lam, _lamF_lam, _frac_error = mag_to_flux_pyphot(
                    self.fil_obj[i], self.ampl[i], self.err[i], AB_if
                )
            else:
                # print(errors)
                _lam, _F_nu, _F_lam, _lamF_lam, _frac_error = mag_to_flux_pyphot(
                    self.fil_obj[i], self.ampl[i], errors[i], AB_if
                )
            lam.append(_lam)
    ...
    def plot_dist_simple(
        ...
        **kwargs,
        ...
        if estimate_densisty > 0:
            ...
            flux = np.zeros(
                [estimate_densisty, self.lib_stell.wavelength.magnitude.shape[0]]
            )
            id_parallax = np.random.permutation(self.par_single_container.shape[1])[
                :estimate_densisty
            ]
            for j in range(estimate_densisty):
                i = id_parallax[j]
                flux[j] = self.lib_stell.generate_stellar_spectrum(
                    self.par_single_container[0, 1],
                    g[i],
                    self.par_single_container[1, 1],
                    Z[i],
                ) / (4 * np.pi * self.par_single_container[2, i] ** 2 * kpc**2)
        else:
            if type(Z) is not float:
                Z = np.median(Z)
            if type(g) is not float:
                g = np.median(g)
            flux = self.lib_stell.generate_stellar_spectrum(
                self.par_single[0], g, self.par_single[1], Z
            ) / (4 * np.pi * self.par_single[2] ** 2 * kpc**2)
        if self.EBV is not None:
        ...
        r_sample = self.lib_stell.get_radius(
            self.par_single[1], self.par_single_container[0]
        )
        d_samples = self.par_single_container[2]
        param = [
            10 ** self.par_single[0],
            10 ** np.quantile(self.par_single_container[0], 0.84)
            - 10 ** self.par_single[0],
            10 ** self.par_single[0]
            - 10 ** np.quantile(self.par_single_container[0], 0.16),
            np.median(d_samples),
            np.median(r_sample) / Rs,
            np.quantile(r_sample / Rs, 0.84) - np.median(r_sample / Rs),
            np.median(r_sample) / Rs - np.quantile(r_sample / Rs, 0.16),
            self.Z,
        ]
        if self.Z is not None:
            label = r"$T={0:.0f}^{{+{1:.0f}}}_{{-{2:.0f}}}$ K, $R={4:.3g}^{{+{5:.2g}}}_{{-{6:.2g}}}$ $R_{{\odot}}$, $Z={7:.3f}$".format(
                *param
            )
        else:
            new_param = [
                *param[:-1],
                self.par_single[3],
                np.quantile(self.par_single_container[3], 0.84)
                - np.median(self.par_single_container[3]),
                self.par_single[3] - np.quantile(self.par_single_container[3], 0.16),
            ]
            label = r"$T={0:.0f}^{{+{1:.0f}}}_{{-{2:.0f}}}$ K, $R={4:.3f}^{{+{5:.2g}}}_{{-{6:.2g}}}$ $R_{{\odot}}$, $Z={7:.4f}^{{+{8:.2g}}}_{{-{9:.2g}}}$".format(
                *new_param
            )
        if estimate_densisty > 0:
            wave_arr = (
                np.array(self.lib_stell.wavelength)
                .reshape(1, -1)
                .repeat(estimate_densisty, 0)
            )
            if use_AB:
                sns.lineplot(
                    x=wave_arr.flatten() / 10**4,
                    y=Flux_to_AB(wave_arr.flatten(), flux.flatten()),
                    ax=ax,
                    label=label,
                    color="orange",
                )
                ...
                sns.lineplot(
                    x=wave_arr.flatten() / 10**4,
                    y=np.log10(flux.flatten() * wave_arr.flatten()),
                    ax=ax,
                    label=label,
                    color="orange",
                )
            ...
            if use_AB:
                ax.plot(
                    self.lib_stell.wavelength.magnitude / 10**4,
                    Flux_to_AB(self.lib_stell.wavelength.magnitude, flux),
                    label=label,
                    color="orange",
                )
            else:
                ax.plot(
                    self.lib_stell.wavelength.magnitude / 10**4,
                    np.log10(np.array(flux) * np.array(self.lib_stell.wavelength)),
                    label=label,
                    color="orange",
                )
        ax.set_xscale("log")
        ...
        if plot_chi2:
            plt.figtext(
                0.2,
                0.05,
                r"$\chi^2$= {0:.1f} N = {1:.0f}".format(self.chis, len(self.filters)),
            )
        if self.gp is None:
            plt.figtext(
                0.2,
                0.1,
                r"$\log(g)={0:.2f}^{{+{2:.2f}}}_{{-{1:.2f}}}$".format(
                    np.median(self.par_single_container[3]),
                    np.median(self.par_single_container[3])
                    - np.quantile(self.par_single_container[3], 0.16),
                    np.quantile(self.par_single_container[3], 0.84)
                    - np.median(self.par_single_container[3]),
                ),
            )
    ...
    def plot_corner_simple(
        ...
        upper = list(
            map(
                lambda x: (n_l + 2) / 2 * np.quantile(x, 0.84)
                - np.quantile(x, 0.16) * n_l / 2,
                self.par_single_container,
            )
        )
        lower = list(
            map(
                lambda x: (n_l + 2) / 2 * np.quantile(x, 0.16)
                - np.quantile(x, 0.84) * n_l / 2,
                self.par_single_container,
            )
        )
        ...
        bins = list(
            map(
                lambda x: int(
                    bi / (upper[x] - lower[x]) * (max(data[:, x]) - min(data[:, x]))
                ),
                range(len(upper)),
            )
        )
    ...
    def plot_dist_double(
        ...
        if ax is None:
            # fig = plt.figure(figsize=(6,4))
            ax = plt.axes()
        r1_sample = self.lib_stell.get_radius(
            self.par_double_container[1], self.par_double_container[0]
        )
        r2_sample = self.lib_stell.get_radius(
            self.par_double_container[3], self.par_double_container[2]
        )
        param1 = [
            10 ** self.par_double[0],
            10 ** np.quantile(self.par_double_container[0], 0.84)
            - 10 ** self.par_double[0],
            10 ** self.par_double[0]
            - 10 ** np.quantile(self.par_double_container[0], 0.16),
            np.median(r1_sample) / Rs,
            np.quantile(r1_sample / Rs, 0.84) - np.median(r1_sample / Rs),
            np.median(r1_sample) / Rs - np.quantile(r1_sample / Rs, 0.16),
        ]
        param2 = [
            10 ** self.par_double[2],
            10 ** np.quantile(self.par_double_container[2], 0.84)
            - 10 ** self.par_double[2],
            pow(10, self.par_double[2])
            - pow(10, np.quantile(self.par_double_container[2], 0.16)),
            np.median(r2_sample) / Rs,
            np.quantile(r2_sample / Rs, 0.84) - np.median(r2_sample / Rs),
            np.median(r2_sample) / Rs - np.quantile(r2_sample / Rs, 0.16),
        ]
        print(param1, param2)
        d_samples = self.par_double_container[4]
        if estimate_density > 0:
            flux1 = np.zeros(
                [estimate_density, len(self.lib_stell.wavelength.magnitude)]
            )
            flux2 = np.zeros(
                [estimate_density, len(self.lib_stell.wavelength.magnitude)]
            )
            for i in range(estimate_density):
                flux1[i] = self.lib_stell.generate_stellar_spectrum(
                    self.par_double_container[0, i],
                    self.g1,
                    self.par_double_container[1, i],
                    self.Z1,
                ) / (4 * np.pi * d_samples[i] ** 2 * kpc**2)
                flux2[i] = self.lib_stell.generate_stellar_spectrum(
                    self.par_double_container[2, i],
                    self.g1,
                    self.par_double_container[3, i],
                    self.Z1,
                ) / (4 * np.pi * d_samples[i] ** 2 * kpc**2)
            wave_arr = np.repeat(
                self.lib_stell.wavelength.magnitude.reshape(1, -1), estimate_density, 0
            )
        else:
            flux1 = self.lib_stell.generate_stellar_spectrum(
                self.par_double[0], self.g1, self.par_double[1], self.Z1
            ) / (4 * math.pi * np.median(d_samples) ** 2 * kpc**2)
            flux2 = self.lib_stell.generate_stellar_spectrum(
                self.par_double[2], self.g2, self.par_double[3], self.Z2
            ) / (4 * math.pi * np.median(d_samples) ** 2 * kpc**2)
        if self.EBV is not None:
        ...
        if estimate_error is None:
            ...
            for i in range(len(self.err)):
                if self.err_estimate_flag[i]:
                    errors[i] = np.sqrt(
                        (val_arr[i, 0] * correction / pred[i] * self.error_estimate[1])
                        ** 2
                        + self.err[i] ** 2
                    )
                else:
            ...
            for i in range(len(self.err)):
                if self.err_estimate_flag[i]:
                    errors[i] = np.sqrt(
                        (val_arr[i, 1] * correction / pred[i] * self.error_estimate[1])
                        ** 2
                        + self.err[i] ** 2
                    )
                else:
        ...
        label1 = r"$T={0:.0f}^{{+{1:.0f}}}_{{-{2:.0f}}}$ K, $R={3:.3g}^{{+{4:.2g}}}_{{-{5:.2g}}}$ $R_{{\odot}}$".format(
            *param1
        )
        label2 = r"$T={0:.0f}^{{+{1:.0f}}}_{{-{2:.0f}}}$ K, $R={3:.3g}^{{+{4:.2g}}}_{{-{5:.2g}}}$ $R_{{\odot}}$".format(
            *param2
        )
        label3 = "combined"
        if use_AB:
            if estimate_density > 0:
                sns.lineplot(
                    x=wave_arr.flatten() / 10**4,
                    y=Flux_to_AB(wave_arr.flatten(), flux1.flatten()),
                    ax=ax,
                    label=label1,
                    color="yellow",
                )
                sns.lineplot(
                    x=wave_arr.flatten() / 10**4,
                    y=Flux_to_AB(wave_arr.flatten(), flux2.flatten()),
                    ax=ax,
                    label=label2,
                    color="red",
                )
                sns.lineplot(
                    x=wave_arr.flatten() / 10**4,
                    y=Flux_to_AB(wave_arr.flatten(), (flux1 + flux2).flatten()),
                    ax=ax,
                    label=label3,
                    color="grey",
                )
                ...
                ax.plot(
                    self.lib_stell.wavelength.magnitude / 10**4,
                    Flux_to_AB(self.lib_stell.wavelength.magnitude, flux1),
                    label=label1,
                    color="yellow",
                )
                ax.plot(
                    self.lib_stell.wavelength.magnitude / 10**4,
                    Flux_to_AB(self.lib_stell.wavelength.magnitude, flux2),
                    label=label2,
                    color="red",
                )
                ax.plot(
                    self.lib_stell.wavelength.magnitude / 10**4,
                    Flux_to_AB(self.lib_stell.wavelength.magnitude, flux1 + flux2),
                    label=label3,
                    color="grey",
                )
        else:
            if estimate_density > 0:
                sns.lineplot(
                    x=wave_arr.flatten() / 10**4,
                    y=np.log10(wave_arr.flatten(), flux1.flatten()),
                    ax=ax,
                    label=label1,
                    color="yellow",
                )
                sns.lineplot(
                    x=wave_arr.flatten() / 10**4,
                    y=np.log10(wave_arr.flatten(), flux2.flatten()),
                    ax=ax,
                    label=label2,
                    color="red",
                )
                sns.lineplot(
                    x=wave_arr.flatten() / 10**4,
                    y=np.log10(wave_arr.flatten(), (flux1 + flux2).flatten()),
                    ax=ax,
                    label=label3,
                    color="grey",
                )
                ...
                ax.plot(
                    self.lib_stell.wavelength.magnitude / 10**4,
                    np.log10(np.array(flux1) * np.array(self.lib_stell.wavelength)),
                    label=label1,
                    color="yellow",
                )
                ax.plot(
                    self.lib_stell.wavelength.magnitude / 10**4,
                    np.log10(np.array(flux2) * np.array(self.lib_stell.wavelength)),
                    label=label2,
                    color="red",
                )
                ax.plot(
                    self.lib_stell.wavelength.magnitude / 10**4,
                    np.log10(
                        np.array(flux1 + flux2) * np.array(self.lib_stell.wavelength)
                    ),
                    label=label3,
                    color="grey",
                )
        ...
        if plot_bic:
            ...
            plt.figtext(
                0.2,
                0.05,
                r"$\chi^2$= {0:.1f} N = {1:.0f}".format(self.chi2d, len(self.filters)),
            )
        if self.EBV is None:
            high = 0.05
        else:
    ...
    def plot_corner_double(
        ...
        upper = [
            (n_l + 2) / 2 * np.quantile(self.par_double_container[0], 0.84)
            - np.quantile(self.par_double_container[0], 0.16) * n_l / 2,
            (n_l + 2) / 2 * np.quantile(self.par_double_container[1], 0.84)
            - np.quantile(self.par_double_container[1], 0.16) * n_l / 2,
            (n_l + 2) / 2 * np.quantile(self.par_double_container[2], 0.84)
            - np.quantile(self.par_double_container[2], 0.16) * n_l / 2,
            (n_l + 2) / 2 * np.quantile(self.par_double_container[3], 0.84)
            - np.quantile(self.par_double_container[3], 0.16) * n_l / 2,
            (n_l + 2) / 2 * np.quantile(self.par_double_container[4], 0.84)
            - np.quantile(self.par_double_container[4], 0.16) * n_l / 2,
        ]
        lower = [
            (n_l + 2) / 2 * np.quantile(self.par_double_container[0], 0.16)
            - np.quantile(self.par_double_container[0], 0.84) * n_l / 2,
            (n_l + 2) / 2 * np.quantile(self.par_double_container[1], 0.16)
            - np.quantile(self.par_double_container[1], 0.84) * n_l / 2,
            (n_l + 2) / 2 * np.quantile(self.par_double_container[2], 0.16)
            - np.quantile(self.par_double_container[2], 0.84) * n_l / 2,
            (n_l + 2) / 2 * np.quantile(self.par_double_container[3], 0.16)
            - np.quantile(self.par_double_container[3], 0.84) * n_l / 2,
            (n_l + 2) / 2 * np.quantile(self.par_double_container[4], 0.16)
            - np.quantile(self.par_double_container[4], 0.84) * n_l / 2,
        ]
